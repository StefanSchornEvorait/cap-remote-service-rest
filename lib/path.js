...
function replacePlaceholder(segment, data) {
  ...
  if (!!paramValue) {
    return segment.replace('{' + paramName + '}', paramValue);
  } else {
...
function isQueryParam(definition, paramName) {
  ...
  if (param?.[OpenApiAnnotation.PARAM_IN] === OpenApiParamIn.QUERY) {
    return true;
  }
  for (const param in definition.params) {
    if (param["@openapi.name"] === paramName && param[OpenApiAnnotation.PARAM_IN] === OpenApiParamIn.QUERY) {
      return true;
    }
  }
}
